...
def xor_contrast(variables: np.ndarray) -> float:
    """
    XOR k张恢复对比度
    :param variables:
    :return:
...
def xor_safety_penalty_list(variables: np.ndarray) -> list:
    """
    安全惩罚值，也就是不足k张异或时的对比度
    :param variables:
    :return: 长度为`n-1`的列表，分别代表 1,...,n-1张异或的对比度
    """
    k = len(variables[0]) - 1
    # security penalty
    ps = [0] * (k - 1)
    ...
    for t in range(1, k):
        ...
        ps[t - 1] = max(0, abs(sum(variables[1] * rt) - sum(variables[0] * rt)) - EPSILON)
    return ps


def xor_penalty(variables: np.ndarray, ws, wc) -> float:
    k = len(variables[0]) - 1
    # security penalty
    sum_ps = sum(safety_penalty_list(variables))
    # contrast penalty
...
def xor_energy(variables: np.ndarray, ws, wc) -> float:
    return xor_contrast(variables) * penalty(variables, ws, wc)
