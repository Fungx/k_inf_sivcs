...
from optimize import OptimizedResult, optimize_sa3
import os
...
def find_best_sa3(times, dest_path, k, markov, esp_p, esp_r, ws):
    start = time.time()
    all_results = []
    best_contrast_1 = 0
    best_time_1 = -1
    ...
    for t in range(times):
        res = optimize_sa3(k, markov=markov * k, ws=ws, initial_temp=0.1, terminated_temp=0.1 / 2000,
...
if __name__ == "__main__":
    ...
    ESP_R, ESP_P = 0.5, 0.5
    K = 3
    dir = f"res_sa3_new2_many_one_k={K}_mk={MARKOV}k_ws={WS}_espr={ESP_R}_espp={ESP_P}_TIMES={TIMES}"
    os.mkdir(dir)
    for t in range(8):
        print(f"start k={K}...")
        p = Process(target=find_best_sa3, args=(TIMES, f"{dir}/{t}.json", K, MARKOV * K, ESP_P, ESP_R, WS))
        p.start()
